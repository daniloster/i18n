...
import useObservableState from '../DEV/useObservableState'
  ...
  return (
    <>
      ...
      <button
        ...
        onClick={() => {
          setState(({ number }) => ({
            number: (prefix.includes('1st') ? 10 : 20) + number,
          }))
        }}
      >
        Increment {prefix}
...
describe('useObservableState', () => {
  ...
  test('if useObservableState share state changed from first internal component', async () => {
    ...
    fireEvent.click(await wrapper.getByTestId('id-1st-Number'))
    expect(await wrapper.findByText(/1st number: 11/i)).toBeInTheDocument()
    expect(await wrapper.findByText(/2nd number: 11/i)).toBeInTheDocument()
  })
  ...
  test('if useObservableState share state changed from first internal component', async () => {
    ...
    fireEvent.click(await wrapper.getByTestId('id-2nd-Number'))
    expect(await wrapper.findByText(/1st number: 21/i)).toBeInTheDocument()
    expect(await wrapper.findByText(/2nd number: 21/i)).toBeInTheDocument()
  })
